...
База — SQLite в памяти, сеть не нужна. В `test_saas.py` класс `TestKnownBugs` — девять ошибок `views_saas.py`, найденных тестами (отрицательные цены, 500 на `'²'` и огромных числах, JSON-массив вместо объекта, лимит столов при регистрации, невалидный e-mail, событие без `venue`). Все исправлены 22.09, тесты остались как регрессионные.
