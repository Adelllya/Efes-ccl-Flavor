...
- **QR-коды столов.** `GET /api/venues/<slug>/qr.svg?table=N` рисует QR со ссылкой
  ...
  Таблички печатаются из панели: вкладка «Меню» (карточка заведения) или «Пилот», кнопка «QR для столов»,
  четыре таблички A6 на листе A4. Сверху панель показывает ссылку из QR (`GET /api/venues/<slug>/qr-link/?table=N`)
  и предупреждает, если это адрес компьютера или локальной сети.
...
- **Цифры пилота.** В панели вкладка «Пилот» (владелец заведения и модератор): воронка, чек, пары, оценки, кнопки CSV.
  Под ней `GET /api/pilot/report/?venue=<slug>&from=ГГГГ-ММ-ДД&to=ГГГГ-ММ-ДД`
