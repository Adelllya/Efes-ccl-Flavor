...
как и раньше. Ежедневный крон `/api/health/` из `vercel.json` заводится в обоих проектах; у фронта он
отвечает ошибкой (базы там нет), это ожидаемо и ни на что не влияет.
...
и предупреждает, если это адрес компьютера или локальной сети. Перед печатью всех табличек отсканируйте одну телефоном.
...
- **Цифры пилота.** В панели вкладка «Пилот» (владелец заведения и модератор): воронка, чек, пары, оценки, кнопки CSV. API: `GET /api/pilot/report/?venue=<slug>&from=ГГГГ-ММ-ДД&to=ГГГГ-ММ-ДД`
...
- Сломался деплой: Vercel → Deployments → предыдущий рабочий → Promote to Production.
  Если прод сломался после пуша в `main`: `git revert <коммит>` и пуш, Vercel за минуту соберёт
  предыдущее состояние. Promote to Production в панели тоже работает, но следующий пуш в `main` его перекроет.
