...
- **Подбор к блюдам меню.** Бар советует только напитки из своей карты, которые в наличии
  (сорта каталога и любые напитки базы подбора, их добавляют в панели «Меню»). Когда меню и карта
  напитков бара заведены, проверьте, что гость увидит у каждого блюда:
  ```
  DATABASE_URL="<строка>" .venv/bin/python manage.py venue_pairing_check --venue <slug>
  ```
  Блюдо, которое не нашлось в базе подбора ни по названию, ни по похожему блюду, лучше назвать
  как в базе (например «Гренки чесночные», «Кальмары в кляре»).
