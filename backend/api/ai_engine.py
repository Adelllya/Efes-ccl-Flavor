...
def clean_reason(text):
    ...
    # В ответе объяснение уже стоит после двоеточия, поэтому одиночное тире становится запятой
    text = re.sub(r'\s*[\u2014\u2013]\s*', ', ', text)
    return lower_first(text.rstrip('. '))


def clean_name(text):
    """Название из данных движка для гостя: длинное тире между словами становится запятой, внутри слова дефисом."""
