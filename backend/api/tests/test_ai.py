...
from api import ai_engine, ai_safety, ai_sommelier, ai_usage
...
class LocalSommelierTests(AiTestsBase):
    def test_drink_for_named_dish_follows_engine(self):
        ...
        top, result = self.engine_ranked(self.besh)[0]
    ...
    def test_celebration_does_not_pick_strongest_by_default(self):
        ...
        if drink is not None:
            ...
            self.assertEqual(drink['id'], self.engine_ranked(menu_item)[0][0]['id'])
    ...
    def test_tea_is_not_pushed_first_without_request(self):
        ...
        if ranked[0][0].get('category') == 'tea' and top_beer and top_beer[1]['score'] >= ranked[0][1]['score'] - 12:
            self.assertEqual(first['id'], top_beer[0]['id'])
        else:
            self.assertEqual(first['id'], ranked[0][0]['id'])
...
class AiRequestValidationTests(AiTestsBase):
    ...
    def test_internal_meta_is_not_returned(self):
        ...
        self.assertEqual(set(data), {'reply', 'suggestions', 'mode', 'note', 'safety', 'disclaimer', 'lang'})
...
class ClaudePathTests(AiTestsBase):
    ...
    def test_remembered_safety_filters_model_answer(self):
        ...
        bad = {'reply': 'Возьмите Хмельной Лось, он к мясу.', 'suggestions': []}
        resp, _ = self.ask_claude('а что к бешбармаку?', bad, history=history)
        data = resp.json()
        self.assertEqual(data['mode'], 'local')
        self.assertNotIn('Хмельной Лось', data['reply'])
    ...
    def test_daily_limit_switches_to_engine(self):
        ...
        # Лимит 0 выключает Claude совсем
...
class CallClaudeTests(TestCase):
    ...
    def test_cost_estimate_by_model(self):
        ...
        self.assertAlmostEqual(ai_usage.cost_usd('claude-opus-5-5', usage), 4.0 + 2.0 + 0.4)
...
class AiEvalCommandTests(TestCase):
    def test_eval_writes_report_and_rolls_back(self):
        ...
        self.assertIn('Пройдено 30 из 30', stdout.getvalue())
        self.assertIn('**30 из 30**', report)
        self.assertEqual(sum(1 for line in report.splitlines() if line.startswith('| ') and line[2].isdigit()), 30)
